...
tasks.register<Copy>("distApk") {
    ...
    rename { "sample-$version-debug.apk" }
    doLast { println(">> APK versionado em: dist/sample-$version-debug.apk") }
}
...
tasks.register<Copy>("distReleaseApk") {
    ...
    into(rootProject.layout.projectDirectory.dir("dist"))
    rename { "sample-$version-release.apk" }
    doLast { println(">> APK versionado em: dist/sample-$version-release.apk") }
}
